...
class QuestionsViewController: UIViewController {
	...
	@IBOutlet weak var answerStub: UIButton!
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		if segue.identifier == "unwindToQRScanner" {
			...
			imageDetailsVC.closeViewButton?.backgroundColor = .themeStyle(dark: .red, light: .red)
	...
	@objc func verifyButton(_ sender: UIButton) {
	...
	@objc func loadCurrentTheme() {
		...
		self.helpButton.setTitleColor(dark: .red, light: .defaultTintColor, for: .normal)
		...
		self.answerButtons.forEach { $0.backgroundColor = .themeStyle(dark: .red, light: .red); $0.dontInvertColors() }
	...
	@objc private func verify(answer: UInt8) {
		...
		UIView.transition(with: self.answerButtons[Int(answer)], duration: 0.25, options: [.transitionCrossDissolve], animations: {
			if willNoticeIfAnswerIsCorrectOrIncorrect {
				self.answerButtons[Int(answer)].backgroundColor = .red
			} else {
                self.answerButtons[Int(answer)].backgroundColor = .themeStyle(dark: .warmYellow, light: .red)
			}
			...
			if completed {
				...
				UIView.transition(with: self.answerButtons[Int(answer)], duration: 0.2, options: [.transitionCrossDissolve], animations: {
					self.answerButtons[Int(answer)].backgroundColor = .themeStyle(dark: .red, light: .defaultTintColor)
				})
			}
		}
	}
